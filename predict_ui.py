...
# # FILE INPUTS
# # =========================
# yesterday_file = st.file_uploader("📂 Upload yesterday's data (CSV)", type=["csv"], key="yesterday")
# history_file   = st.file_uploader("📜 Upload historical data (CSV: trainset or healthy subset)", type=["csv"], key="history")

# df_current = load_csv(yesterday_file, DEFAULT_YESTERDAY, "yesterday data")
...
# except Exception as e:
#     st.warning(f"⚠️ Could not load threshold file '{THRESHOLD_FILE}': {e}. Falling back to 0.5.")
#     threshold = 0.5
...
# - **Battery (V)**: < **3600** = red, < **3800** = orange  
# - **RSSI (abs)**: > **50** = red, > **40** = orange
#         """
#     )

# # =========================
# # Adaptive precision guardrail using ground-truth if provided
# # =========================
# st.markdown("---")
...
#     # Optional: suggest a higher threshold that keeps precision ≥ 0.45 (only if GT provided)
...
#     best_thr, best_f1 = threshold, 0.0
...
#     if best_f1 > 0:
#         st.info(f"⚙️ Suggested higher precision threshold: {best_thr:.3f} (F1={best_f1:.3f})")
...
MODEL_FILE = "model_behavior_aware.pkl"
THRESHOLD_FILE = "threshold_behavior_aware.npy"
DEFAULT_YESTERDAY = "yesterday.csv"       # not auto-used anymore
DEFAULT_HISTORY = "one_trainset.csv"      # still allowed as default
...
# *** IMPORTANT: require uploading Yesterday CSV; do NOT auto-load DEFAULT_YESTERDAY ***
if not yesterday_file:
    st.info("➡️ Please upload the Yesterday CSV to run predictions. No predictions are shown by default.")
    st.stop()
...
# =========================
# Evaluate With Ground Truth (optional)
# =========================
st.markdown("---")
st.subheader("Evaluate With Ground Truth")
truth_file = st.file_uploader("📂 Upload Ground Truth CSV (DeviceReferenceId of actual disconnects)", type=["csv"], key="truth")
